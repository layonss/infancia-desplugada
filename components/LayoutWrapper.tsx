  ...
  return (
    <Providers>
      <div
        ...
        style={isHome ? { backgroundImage: "url('/imagens/abacoplay.jpeg')" } : undefined}
      >
        {/* NAVBAR */}
        <header
          className={`w-full transition-all duration-300 ${
            isHome
              ? "absolute top-0 left-0 z-50 border-b border-white/10 bg-black/10 backdrop-blur-sm text-white"
              : "sticky top-0 z-50 border-b border-slate-200/80 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md text-slate-700 dark:text-slate-200 shadow-sm"
          }`}
        >
          <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
            ...
            <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
              ...
              <div className={`pl-6 border-l ${isHome ? "border-white/20" : "border-slate-200 dark:border-slate-700"}`}>
                 <ThemeToggle />
              </div>
            </nav>
        ...
        <footer className={`w-full border-t transition-colors duration-300 ${
          isHome ? "bg-black/80 backdrop-blur-md text-white border-white/10" : "bg-slate-900 dark:bg-slate-950 text-slate-300 border-slate-800"
        }`}>
          <div className="max-w-6xl mx-auto px-6 py-12">
            <div className="grid md:grid-cols-4 gap-8 mb-12">
              <div className="md:col-span-1 space-y-4">
                <h3 className="text-lg font-bold text-white tracking-tight">Infância (Des)plugada</h3>
                <p className="text-sm leading-relaxed opacity-80">Projeto de práticas pedagógicas focado no pensamento computacional sem o uso de telas.</p>
              </div>

              <div className="space-y-4">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider opacity-60">Explorar</h3>
                <nav className="flex flex-col space-y-3 text-sm">
                  {[
                    ...
                    <Link key={item.path} href={item.path} className="group flex items-center text-slate-400 hover:text-indigo-400 transition-all duration-300">
              ...
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider opacity-60">Coordenação</h3>
                <div className="text-sm space-y-2 opacity-80">
                  <p>Iniciação Científica</p>
                  <p>Sistemas de Informação</p>
                  <p className="pt-2 text-xs">
                    <span className="opacity-50 block mb-1">Orientação:</span> Prof. Dra. Jéssica Zacarias
                  </p>
                </div>
              </div>

              <div className="md:col-span-1 space-y-4">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider opacity-60">Realização & Apoio</h3>
            ...
            <div className={`pt-8 border-t flex flex-col md:flex-row justify-between items-center text-xs opacity-60 ${isHome ? "border-white/20" : "border-slate-800"}`}>
              <p>© {new Date().getFullYear()} Infância (Des)plugada.</p>
              <p>Desenvolvido por <span className="font-bold text-white">Layon Silva</span></p>
            </div>
          </div>
        </footer>
